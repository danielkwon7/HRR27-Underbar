(function() {
  ...
  _.last = function(array, n) {
    var number = array.length - n;
    return n === undefined ? array[array.length-1] : array.slice(number >= 0 ? number : 0);
  };
  ...
  _.reject = function(collection, test) {
    ...
    for (var i = 0; i <= collection.length - 1; i++) {
      if (!opposite.includes(collection[i])) {
        result.push(collection[i]);
      }
    }
    return result;
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (isSorted) {
      for (var i = 0; i <= array.length - 1; i++) {
        ...
        if (iterator) {
          if (!iterated.includes(iterator(value))) {
            result.push(value);
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var start = accumulator != undefined ? accumulator : collection[0];
    var index = accumulator != undefined ? 0 : 1;
    for (var i = index; i <= collection.length - 1; i++) {
  ...
  _.shuffle = function(array) {
    ...
    for (var i = 0; i < index; i++) {
      var transferIndex = randomNumber();
      var temporaryValue = copiedArray[transferIndex];
      copiedArray[transferIndex] = copiedArray[i];
      copiedArray[i] = temporaryValue;
    }
    return copiedArray;
